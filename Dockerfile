...
# Copy all files
COPY /src/nhsuk.base-application/*.csproj .
...
RUN dotnet build *.csproj --configfile nuget.config

# Use native linux file polling for better performance
ENV DOTNET_USE_POLLING_FILE_WATCHER 1
ENTRYPOINT dotnet watch -p src/nhsuk.base-application/*.csproj run --urls=http://+:5000 --no-restore
